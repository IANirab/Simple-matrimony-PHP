...
if ((isset($_GET["b"])) && is_numeric($_GET["b"]) && $_GET["b"]!=0)
	...
	if ($action == 0)
	{
	 	...
	 	$sSql = "insert into tbldating_district_master (title,state_id,languageid) values($query)";
		$retfile ="districtmaster.php";
	}
	else
	{
		//$sSql = "update tbldating_city_master set $query,modifydate=curdate() where id=$action";
	 $sSql = "update tbldating_district_master set $query where id=$action";
	$retfile ="districtmanager.php";
	}
execute($sSql);
