...
<div class="pagewrapper">
	<div class="container">
		...
		<div class="col-lg-9 center_right">
			<div id="center-in">
			<!-- ********* TITLE START HERE *********-->
			<h1 class="pagetitle">Caste Manager</h1>
			<div class="addlink1">
			<? if($cast_mgmnt_cm_2 == 'Y' || $cast_mgmnt_cm_2=='N') { ?>			
			<div class="addlink"><a href="castmaster.php">Add new caste</a></div>
			<? } ?>
            ...
            <form method="post" action="cast_query.php" class="form_second" name="cast">
            <div class="form-group">
             <label>Search</label>
           
            
             
              <select name="cmbreligian" class="form-control">
<? fillcombo("select id,title from tbldatingreligianmaster where currentstatus=0","","Select Religion"); ?>
               ...
               <div class="form-group">
              <label>&nbsp;</label>
               <input type="text" name="cast"  id="cast" value="" class="form-control">
               </div>
...
$result = getdata("select tbldatingcastmaster.id,tbldatingcastmaster.title,tbldatingsitelanguagemaster.language,tbldatingreligianmaster.title ". $fromqry ." order by tbldatingreligianmaster.title,tbldatingcastmaster.title ". $NoOfRecord);
while($rs= mysqli_fetch_array($result))
{
			...
			<td ><?=$religian?></td>
          	<td ><?=$title?></td>
          	<td class="class="mobile_none ""><?=$language?></td>
            <td >
				<? if($cast_mgmnt_cm_2 == 'Y' || $cast_mgmnt_cm_2 == 'N') {
 ?>				
		    	<a href="castmaster.php?b=<?= $id ?>" class="actionbtn_m green">Modify</a> 
				<? } if($cast_mgmnt_cm_4 == 'Y' || $cast_mgmnt_cm_4 == 'N') { ?>
				<a href="castdelete.php?b=<?= $id ?>" class="actionbtndel">Delete</a>
				<? } ?>
            	</td>
            </tr>
		<?
