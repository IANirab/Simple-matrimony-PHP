...
<div class="pagewrapper">
	<div class="container">
		...
		<div class="col-lg-9 center_right">
			<div id="center-in">
			<!-- ********* TITLE START HERE *********-->
			<h1 class="pagetitle">Disapprove Education Manager</h1>
			<div class="addlink1">
			<?php /*?><?
...
$fromqry = " from tbl_education_master where currentstatus in (5) ";
$fromqry .= $searchqry;
...
$result = getdata("select id,title,cat_id ". $fromqry ." order by tbl_education_master.id desc ". $NoOfRecord);
while($rs= mysqli_fetch_array($result))
{
		  	$LocationId=$rs[0];
			$LocationName=$rs[1];
			$cat_id=$rs[2];
			if($cat_id!='' && $cat_id!='0.0')
			{
				$cat_id=getonefielddata("select title from tbl_education_category_master where id=".$cat_id."");
			}
			/*$imagenm=$rs[2];
			$language=$rs[3];
			if ($imagenm == "")
		 ...
		 ?>
			...
			<td >
				...
				<a href="other_education_delete.php?b=<?= $LocationId ?>" class="actionbtndel green">Approve</a>
				</td>
            </tr>
		<?
	}
