...
<div class="pagewrapper">
	<div class="container">
		...
		<div id="center-in">
			<!-- ********* TITLE START HERE *********-->
			<h1 class="pagetitle" > <i class="fas fa-theater-masks"></i> Recent Profile of The Week Lists
		<!-- CENTER END ######################## -->
	</div>	
      ...
      if(isset($_GET['did'])){
        ...
        $id=$_GET['did'];
        $sql = "delete from tbl_profile_week where profileid='$id'";
        $result = $db->query($sql);
        
            if($result){
                
                echo "<script type='text/javascript'>window.location.href='addprofileweek.php';</script>";
      ...
      ?>
        ...
        $db = new mysqli("localhost","paroshic_paroshic","kxmcwQzLTrTR","paroshic_matri2018jl");
        $sql = "select * from tbl_profile_week";
        $result = $db->query($sql);
        ...
        $sql1 = "select * from  tbldatingusermaster where userid='$profileid'";
        ...
        ?>        
              <tr>
                ...
                <a href="https://www.paroshi.com/displayprofile.php?b=<? echo $data1->userid; ?>" target="_blank">
                <button type="button" class="btn btn-xs btn-primary">View
                </button>
                </a>
                
                <br><br>
                
                <a href="?did=<? echo $data1->userid; ?>">
...
<!-- User-List-Start -->
	<div id="center-in">
			<h1 class="pagetitle" > <i class="fas fa-theater-masks"></i> All User Lists
	</div>	
          <table class="table table-bordered">
            <thead>
              <tr>
    ...
    <?
      if(isset($_GET['id'])){
            ...
            $db = new mysqli("localhost","paroshic_paroshic","kxmcwQzLTrTR","paroshic_matri2018jl");
            $id=$_GET['id'];
             $sql = "INSERT INTO tbl_profile_week (profileid)
                    VALUES ('$id')";
            $result = $db->query($sql);
      ...
      ?> 
        ...
        $db1 = new mysqli("localhost","paroshic_paroshic","kxmcwQzLTrTR","paroshic_matri2018jl");
        $sql1 = "select * from  tbldatingusermaster";
        $result1 = $db1->query($sql1);
        
        while($data1 = $result1->fetch_object()){
            
            if($profileid != $data1->userid){
        ...
        ?>       
              ...
              <tr>
                ...
                <a href="?id=<? echo $data1->userid; ?>">
                <button type="button" class="btn btn-xs btn-danger">ADD
                </button>
                </a>
