...
<? if($print_profile=='Y'){?>
   ...
   <? } ?>         
			...
			<?= $edit_link ?>  <?= $acceptlink ?>  <?= $declinedlink ?>
                ...
                ?>
                <span class="resultRoundName resultRoundName_c"><?=$name?></span> 
            <strong class="pid"><?= $displayprofileprofile_code ?>   <?=$profile_code ?></strong>
            <strong>
            <? $lastlogin=find_lastlogin($dispayuserid); ?>
            	<span class="lastlogin"><?= $displayprofilelastlogin ?> :
                 <?=	$lastlogin ?></span>
                </strong>
            </div>
        	<div class="full_contacter">
            	<div class="searchgridimage">
  <? $get_bigimg=getonefielddata("select imagenm from tbldatingusermaster where userid='".$dispayuserid."' ") ?>                
                	

                    <div class="displayprofileimage">
					<a href="<?= $sitepath ?>uploadfiles/<?=$get_bigimg?>" rel="example_group" title="Title" class="thumbnail">
					<img src="<?= $sitepath ?>uploadfiles/<?=$imagenm?>" height="133" width="100" border="0" >
					</a>
                    </div>
	...
	<? if(isset($_SESSION[$session_name_initital."memberuserid"]) && $_SESSION[$session_name_initital."memberuserid"]!=''){?>
    			<a href="<?= $sitepath ?>uploadfiles/<?=$get_bigimg?>" rel="example_group" title="Title">
  <img src="<?= $siteimagepath ?>images/zoomin.png" /></a>

                                
                <? }else{ ?>
         <a href="<?=$sitepath?>login.php" target="_blank"><img src="<?= $siteimagepath ?>images/zoomin.png"/></a>
                <? } ?>
                </div>
                </div>
 ...
 <div class="display_tabs">
  <ul class="nav nav-tabs">
    ...
    <li><a data-toggle="tab" href="#display_tab2"><?=$displayprofilehead4?></a></li>
     <li><a data-toggle="tab" href="#display_tab3"><?=$displayprofilehead5?></a></li>
  ...
  <div class="tab-content">
               ...
               <li>
				...
				<? } ?>
        			...
        			<!-- tab2 code end -->
         			...
         			<!-- tab3 code start -->
        <div id="display_tab3" class="tab-pane fade">
          <div class="display_tab_info">
        
          <h3><?=$displayprofilehead5?></h3>
        ...
        <div id="display_tab7" class="tab-pane fade">
  		...
  		<? if ($partner_genderid != "") { ?>
                ...
                <? if ($partner_genderid != "") { ?>
                <li>
                <strong><?= str_replace(":","",$displayprofile_partnergender) ?></strong>
                <span>: <?=$partner_genderid?> </span>
				</li>
				<? } ?>
...
<?php /*?><div><?= $updateintrestprofilebrother_married2 ?></div>
<div><? if ($brother_married2 != "") { ?><?= $brother_married2 ?><? } ?></div>
<div><? if ($brother_unmarried2 != "") { ?><?= $brother_unmarried2 ?><? } ?></div>
<?php */?><!-- ROW--->
...
<?php /*?><div><?= $updateintrestprofilesister2 ?></div>
<div><? if ($sister_married2 != "") { ?><?= $sister_married2 ?><? } ?></div>
<div><? if ($sister_unmarried2 != "") { ?><?= $sister_unmarried2 ?><? } ?></div>
<?php */?>
...
<label><?= $registercaptcha ?></label>
	<img style="vertical-align:middle" id="imagenmcaptcha" src='<?= $sitepath ?>uploadfiles/<?= $imagenm_captcha ?>'> &nbsp;
    <input type="hidden" name="hiddencaptcha" id="hiddencaptcha" value="<?=$captch_no?>" />
	<input type="text" name="Captcha"  id="Captcha"  class="form_new" value="" size="3" style="vertical-align:middle" title="<?= $registercaptcha_title ?>" onMouseOver="showToolTip(event,'<?=$captcha_registration?>');return false" onMouseOut="hideToolTip()" onFocus="showToolTip(event,'<?=$captcha_registration?>');return false" onBlur="hideToolTip()"> <?php /*?>* <img src="uploadfiles/refresh.png" onclick="change_captcha();" /> Click to Change<?php */?>
<!-- Code ends Here -->
