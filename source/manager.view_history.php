<!-- ********* TITLE START HERE *********-->
		<div class="pagetitle"><h1><?= $profile_viewpgtitle ?></h1></div>
		<!-- ********* TITLE END HERE *********-->
...
<?
			$FileNm = $sitepath . "views_history.php";
			if (isset($_GET["pgnm"]))
				$Pgnm = $_GET["pgnm"];
			else
				$Pgnm = 1;
			$fromqry = "from tbldatingprofilehistorymaster,tbldatingusermaster where tbldatingprofilehistorymaster.touserid=tbldatingusermaster.userid and  tbldatingprofilehistorymaster.currentstatus In (0,1) and tbldatingusermaster.currentstatus=0 and tbldatingprofilehistorymaster.fromuserid=$curruserid AND tbldatingusermaster.genderid!=$gender group by tbldatingprofilehistorymaster.touserid,age,religianid,castid,state,city,countryid ";
		...
		$result = getdata("select count(historyid), tbldatingprofilehistorymaster.touserid," . findage() . ",religianid,castid,state,city,countryid,heightid,maritalstatusid,educationid,ocupationid $fromqry $NoOfRecord");
...
$imagenm = find_user_image($UserId,"","","");
			?>
            	<tr valign="top">
                	<td class="fav_bg" height="35" colspan="4">
				...
				<td class="gritem1">
			    <img align="absmiddle" src="<?= $siteimagepath ?>images/displayprofileicon.gif" border="0" alt="" /><?= setdisplayprofilelink($UserId,$favoriteaction2) ?>
				
				<? if (enable_communication=='Y'){ ?><br />
