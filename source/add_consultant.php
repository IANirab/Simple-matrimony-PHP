...
?>
        ...
        $sql = "select * from tbl_consultant_request where userid='$curruserid' AND status='waiting'";
        $result = $db->query($sql);
        while($data = $result->fetch_object()){
            $userid = $data->userid;
        if($userid){
            echo "<script>alert('Wait Please ! Your Request is now pending !!');</script>";
        ...
        while($data = $result1->fetch_object()){
            ...
            if($packid <= 28){
                 echo "<script>alert('Please Upgrade Your Profile For Use Consultant !!');</script>";
                 echo "<script>window.location.href='dashboard.php';</script>";
            } elseif($consultant == 0){
                echo "<script>alert('Already You Used All Consultant !!');</script>";
                echo "<script>window.location.href='dashboard.php';</script>";
            }
        }
        ?>
...
<head>
    ...
    <title>Request For Consultant :: Paroshi.com</title>
...
<?
if(isset($_POST['consultantsubmit'])){
    $hours = $_POST['hours'];
    $time = $_POST['time'];
    $date = $_POST['date'];
    $conid = $_GET['id'];
    ...
    if($hours == "" || $time == "" || $date == ""){
        ...
        $db = new mysqli("localhost","paroshic_paroshic","kxmcwQzLTrTR","paroshic_matri2018jl");
        $sql = "INSERT INTO tbl_consultant_request(userid,conid,date,time,hours) VALUES('$curruserid','$conid','$date','$time','$hours')";
        $result = $db->query($sql);
...
?>
      ...
      <label for="inputState">Select Hour <i class="fas fa-stopwatch"></i></label>
      <select name="hours" id="inputState" class="form-control">
...
Note : You cannot use 12-Hours Consultant in one time !!
</div>
...
<? } ?>
  ...
  <div class="form-group">
    <label for="formGroupExampleInput">Set Your Time</label>
    <input name="time" type="text" class="form-control" id="formGroupExampleInput" placeholder="Example: 12:00PM to 4PM">
  </div>
    
                                
                                <label class="m-t-15">Select Date <i class="fas fa-calendar-alt"></i></label>
                            ...
                            <div class="border-top">
                                <div class="card-body">
                                    <button type="submit" name="consultantsubmit" class="btn btn-primary">Submit</button>
                                </div>
                            </div>
                        </div>
