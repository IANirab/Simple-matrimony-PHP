...
<head>
    ...
    <title>Viewed My Profile :: Paroshi.com</title>
    <!-- Custom CSS -->
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-12 d-flex no-block align-items-center">
                        <h4 class="page-title">USERS WHO VIEWED MY PROFILE</h4>
                        <div class="ml-auto text-right">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">USERS WHO VIEWED MY PROFILE</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
...
<div class="card">
     ...
     <?
			...
			$FileNm = $sitepath . "viewedmyprofile.php";
			if (isset($_GET["pgnm"]))
				$Pgnm = $_GET["pgnm"];
			else
				$Pgnm = 1;
				
			$fromqry = "from tbldatingprofilehistorymaster,tbldatingusermaster where  tbldatingprofilehistorymaster.touserid=$curruserid and  tbldatingprofilehistorymaster.currentstatus =0 and tbldatingusermaster.currentstatus=0 and tbldatingprofilehistorymaster.fromuserid = tbldatingusermaster.userid AND tbldatingusermaster.genderid!=".$genid."  group by touserid order by historyid DESC ";
		...
		while($rs= mysqli_fetch_array($result))
		{
		 ...
		 $area = $rs['area'];
		 $state = $rs['state'];
		 if($rs['city']!="" && $rs['city']!="0.0" && is_numeric($rs['city'])){
		...
		$imagenm = find_user_image($userid,"","","");
			?>
                ...
                <tr>
                    <td><?=setdisplayprofilelink($userid,$imagenm); ?></td>
                    <td>
                        <? if($age != '' && $heightid!= ''){?>
                        <b><?= $express_intrest_receivedhead02age ?> / <?= $express_intrest_receivedhead02height ?> : </b><?= $age ?> <?= $heightid ?> <br>
                        
                        <? } else if($age!= '' ){ ?>
                        <b><?= $express_intrest_receivedhead02age ?> : </b><?= $age ?> <?= $castid ?><br>
                        
                        <? } else if($heightid!= '' ){ ?>
                        <b><?= $express_intrest_receivedhead02height ?> :</b> <?= $heightid ?> <?= $castid ?><br />
                        
                        <? }if($maritalstatusid != ''){?>
                        <b><?= $express_intrest_receivedhead03 ?></b> <?= $maritalstatusid ?><br />
                        
                        <? } if($castid != ''){?>
                        <b><?= $express_intrest_receivedhead04 ?></b> <?= $castid ?><br>
                        
                        <? } if($religionid != ''){?>
                        <b><?= $express_intrest_receivedhead05 ?></b> <?= $religionid ?><br />
                        <? } ?>
                        
                        <? if($city != ''){?>
					<b><?= $express_intrest_receivedhead06 ?></b> <?= $city ?><br />
					
					<? } if($countryid != ''){?>
                	<b><?= $express_intrest_receivedhead07 ?></b> <?= $countryid ?><br />
                <? } if($education != ''){?>
                	<b><?= $express_intrest_receivedhead08 ?></b> <?= $education ?><br />
                <? } if($occupation != ''){?>
...
if(enable_communication=='Y'){ ?>
                <td><i class="far fa-user"></i> <?= setdisplayprofilelink($userid,$image_on_request_receivedaction2) ?>
                </td><? } ?>
