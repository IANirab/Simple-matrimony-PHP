...
{    
    ...
    if (isset($_SESSION['meChat_logged']) && $_SESSION['meChat_logged'] == true) {
        ...
        $query = "UPDATE `users` SET `lastupdate`= :timestamp WHERE (`id` = :me);";
        $stmt  = $mechat_db->prepare($query);
...
{    
    ...
    if ($stmt->execute() && count($result = $stmt->fetchAll(PDO::FETCH_ASSOC)) > 0) {
        ...
        $query = "UPDATE `users` SET `".$property."`= :value WHERE (`id` = :id);";
        $stmt  = $mechat_db->prepare($query);
...
function meChat_sendMessage($from_chatID, $to_chatID, $message)
{
    ...
    $query = "SELECT * FROM `users` WHERE (`id` = :from) OR (`id` = :to);";
    $stmt  = $mechat_db->prepare($query);
...
function meChatLogged_Friends($page = false, $max_results = 15)
{
    ...
    if (($me = meChatLogged()) != false) {
        ...
        $query = "SELECT `friends`.*,`users`.`name` AS `friend_name`,`users`.`picture` AS `friend_picture`,`users`.`lastupdate` AS `friend_lastupdate` FROM ( SELECT `friendships`.`id` AS `friendship_id`, IF(`id_user_1` != :me , `id_user_1`, `id_user_2`) AS `friend_id` FROM `friendships` WHERE (`id_user_1` = :me OR `id_user_2` = :me) ".$limit.") `friends` INNER JOIN `users` ON `users`.`id` = `friends`.`friend_id`;";
        $stmt  = $mechat_db->prepare($query);
