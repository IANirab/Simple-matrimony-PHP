var tab_opened = meChatTab_Add('Opened'); // Tab opened
var tab_friends = meChatTab_Add('Friends'); // Tab friends
var tab_settings = meChatTab_Add('Settings'); // Tab settings
...
var tab_settings_item = meChatTab_AddItem(tab_settings,
	'<label><input type="checkbox" name="checkbox" value="value" ' + (meChatCookie_getProp('disable_notifications') ? 'checked' : '') + '> Disable notifications</label>'
);
...
meChatTab_ItemFunction(tab_settings_item, 'click touchstart', function(itemID, itemDOM)
{
	...
	meChatTab_ItemHTML(itemID, '<label><input type="checkbox" name="checkbox" value="value" ' + (meChatCookie_getProp('disable_notifications') ? 'checked' : '') + '> Disable notifications</label>');
...
tab_settings_item = meChatTab_AddItem(tab_settings,
	'<label><input type="checkbox" name="checkbox" value="value" ' + (meChatCookie_getProp('disable_sounds') ? 'checked' : '') + '> Disable sounds</label>'
);
...
meChat_Event('chat.open chat.close chat.sendmessage chat.receivemessage chat.body.click', function(chatID, chatDOM, chatProp)
{
	...
	meChatTab_Rename(tab_opened, 'Opened (' + (meChat_openedCount()) + ')'); // Update the tab (Opened) with the amount of open chatboxes
